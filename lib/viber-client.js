...
ViberClient.prototype.postMessage = function(fromPAAdminId, messageType, messageData, optionalTrackingData) {
  if (!toViberUserId || !messageType || !messageData) {
    throw new Error(`Invalid arguments passed to sendMessage. to, type and message are required`);
  }

  const request = { // needs admin user to post to PA
    "from": fromPAAdminId,
    "sender": {
      "name": this._bot.name,
      "avatar": this._bot.avatar
  ...
  this._logger.debug("Sending %s message to viber user '%s' with data", messageType.toUpperCase(), toViberUserId, messageData);
  return this._sendRequest("postPublicAccount", Object.assign(request, messageData));
};
