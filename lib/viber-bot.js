...
function ViberBot(logger, configuration) {
	...
	this._middleware = new Middleware(logger, new MessageValidator(logger, this.authToken));
...
ViberBot.prototype._setupConversationStartedHandler = function() {
	...
	this.on(EventConsts.CONVERSATION_STARTED, response => {
		_.each(self._callbacks[EventConsts.CONVERSATION_STARTED], callback => {
			callback(response.userProfile, responseMessage => {
				...
				const jsonMessage = JSON.stringify(responseMessage.toJson());
				self._middleware.getStream().write(jsonMessage);
			});
		});
	});
};
